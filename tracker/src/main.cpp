...
static const int LOST_FRAME_THRESHOLD = 5;
...
// Helper function: check if bounding box center is outside frame boundaries
static bool isBboxOutsideFrame(const cv::Rect& bbox, int frameW, int frameH) {
    ...
    // Check if center is outside frame, or if bbox is mostly outside
